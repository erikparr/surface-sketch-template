    ...
    *current { ^instance }
    ...
    getTargetInstances { |groupName=nil|
        ...
        if (isAllInstances) {
            ...
            // FIXED: Handle both String and Symbol group names
            var groupKey = groupName;
            if (groups[groupKey].isNil) {
                // Try converting to string if it's a symbol
                if (groupName.isKindOf(Symbol)) {
                    groupKey = groupName.asString;
                } {
                    // Try converting to symbol if it's a string
                    groupKey = groupName.asSymbol;
                };
            };
